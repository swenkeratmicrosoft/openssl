                                              ...
                                              const char *propq)
{
    ...
    if (pem_name != NULL) {
            ...
            *matchcount = 1;
            ...
            if ((slen = pem_check_suffix(pem_name, "PRIVATE KEY")) > 0
                && (ameth = EVP_PKEY_asn1_find_str(NULL, pem_name,
                                                   slen)) != NULL) {
                *matchcount = 1;
                pkey = d2i_PrivateKey_ex(ameth->pkey_id, NULL, &blob, (long)len,
                                         libctx, propq);
            }
        }
    } else {
        ...
        for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
            ...
            ERR_set_mark(); /* prevent flooding error queue */
            tmp_pkey = d2i_PrivateKey_ex(ameth->pkey_id, NULL, &tmp_blob, (long)len,
                                         libctx, propq);
            if (tmp_pkey != NULL) {
                if (pkey != NULL)
                    EVP_PKEY_free(tmp_pkey);
                                          ...
                                          const char *propq)
{
    ...
    if (slen > 0) {
        ...
        if (EVP_PKEY_set_type_str(pkey, pem_name, slen)
            ...
            && ameth->param_decode(pkey, &blob, (int)len))
            ok = 1;
    } else {
        int i;
        EVP_PKEY *tmp_pkey = NULL;
        ...
        for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
            ...
            if (EVP_PKEY_set_type(tmp_pkey, ameth->pkey_id)
                ...
                && ameth->param_decode != NULL
                && ameth->param_decode(tmp_pkey, &tmp_blob, (int)len)) {
                if (pkey != NULL)
                    EVP_PKEY_free(tmp_pkey);
                else
                    pkey = tmp_pkey;
